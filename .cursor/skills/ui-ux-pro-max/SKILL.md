...
> **Distribution hub note:** This skill lives in `agent-skill-set` as an optional vendoring source. Copy `.cursor/skills/ui-ux-pro-max/` into consumer repos that need UI/UX guidance. **Set the target stack per consumer repo** (e.g. `nextjs`, `html-tailwind`, `shadcn`, `react-native`, `swiftui`) — do not assume a single stack here. In each consumer repo, product `DESIGN_GUIDELINES` (or equivalent) remain the authority over this skill.
...
- **Stack**: Set per consumer repo — use `--stack <name>` matching the project's UI framework (see [Available Stacks](#available-stacks))
...
```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```
...
Set `--stack` to match the consumer repo's UI framework. Stacks are defined in `data/stacks/*.csv`.

| Stack | Typical use |
|-------|-------------|
| `nextjs` | Next.js App/Pages Router, SSR, routing |
| `react` | React SPA, hooks, performance |
| `html-tailwind` | Static HTML + Tailwind CSS |
| `shadcn` | shadcn/ui components on React/Next |
| `vue` | Vue 3 composition API |
| `nuxtjs` / `nuxt-ui` | Nuxt 3 apps |
| `svelte` | Svelte/SvelteKit |
| `angular` | Angular components and routing |
...
| `javafx` | Enterprise desktop (AtlantaFX, FXML) |
| `threejs` | WebGL/3D experiences |
| `laravel` | Laravel Blade/Livewire |
| `avalonia` / `wpf` / `winui` / `uwp` | Desktop (.NET) |
| `uno` | Uno Platform |

**Examples:**

```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "dashboard layout forms" --stack nextjs
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "component tokens dark mode" --stack shadcn
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "list navigation performance" --stack react-native
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "atlantafx primer enterprise theme" --stack javafx
```
...
- Stack: `nextjs` (consumer repo uses Next.js + Tailwind)
...
```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "layout routing performance" --stack nextjs
```
